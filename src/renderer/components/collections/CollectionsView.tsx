...
import type { Collection, CollectionDetail, Plant } from '../../types'
...
export default function CollectionsView({ id, navigate }: CollectionsViewProps) {
  const [viewMode, setViewMode] = useState<ViewMode>(id ? 'detail' : 'list')
  ...
  async function loadDetail(collectionId: number) {
    setLoading(true)
    try {
      const data = await window.api.getCollectionById(collectionId)
      setDetail(data)
      setViewMode('detail')
    } catch {
      setDetail(null)
  ...
  async function saveCollection() {
    ...
    try {
      ...
      if (viewMode === 'edit' && editingId) {
        await window.api.updateCollection(editingId, data)
        await loadDetail(editingId)
      } else {
        const result = await window.api.createCollection(data)
        await loadDetail(result.id as number)
      }
      resetForm()
    } catch (err) {
  ...
  async function deleteCollection(collId: number) {
    try {
      await window.api.deleteCollection(collId)
  ...
  async function removePlant(plantId: number) {
    if (!detail) return
    try {
      await window.api.removePlantFromCollection(detail.id, plantId)
  ...
  function renderDetail() {
    ...
    return (
      <div className="animate-fade-in">
        <button onClick={() => { setDetail(null); setViewMode('list'); loadCollections() }} className="btn-ghost mb-4 inline-flex items-center gap-1">
          {'\u2190'} All Collections
        </button>
        ...
        <div className="rounded-2xl p-6 mb-6"
          ...
          <div className="flex items-start justify-between">
            ...
            <div className="flex items-center gap-2">
              <button onClick={() => startEdit(detail)} className="btn-ghost text-xs">
                Edit
              </button>
        ...
        {detail.plants.length === 0 ? (
          <div className="text-center py-12">
            ...
            <button onClick={() => navigate({ view: 'plants' })} className="btn-primary mt-4">
              Browse Plants
            </button>
          </div>
        ) : (
